...
func init() {
	vector.RegisterStore(vector.StoreTypeRedis, func(config *vector.VectorConfig) (vector.VectorStore, error) {
...
func NewRedisVectorStore(config *vector.VectorConfig) (*RedisVectorStore, error) {
...
func (s *RedisVectorStore) SimilaritySearch(ctx context.Context, queryEmbedding []float32, limit int, threshold float64) ([]*vector.SimilarityResult, error) {
...
func (s *RedisVectorStore) SimilaritySearchWithFilter(ctx context.Context, queryEmbedding []float32, filters map[string]interface{}, limit int, threshold float64) ([]*vector.SimilarityResult, error) {
	...
	if err != nil {
		if strings.Contains(err.Error(), "no such index") {
			return []*vector.SimilarityResult{}, nil
		}
...
func (s *RedisVectorStore) StoreBatchEmbeddings(ctx context.Context, embeddings []*vector.EmbeddingData) error {
...
func (s *RedisVectorStore) CreateCollection(ctx context.Context, name string, dimension int, config *vector.CollectionConfig) error {
	dist := "COSINE"
...
func (s *RedisVectorStore) GetCollectionInfo(ctx context.Context, name string) (*vector.CollectionInfo, error) {
	idxName := fmt.Sprintf("idx:%s", name)
