// Package openrouter - OpenRouter embedding provider
...
func init() {
	vector.RegisterEmbeddingProvider(vector.EmbeddingProviderOpenRouter, func(config map[string]interface{}) (vector.EmbeddingProvider, error) {
		apiKey, _ := config["api_key"].(string)
		model, _ := config["model"].(string)
		siteURL, _ := config["site_url"].(string)
		appName, _ := config["app_name"].(string)
		
		return NewOpenRouterEmbeddingProvider(OpenRouterConfig{
			APIKey:  apiKey,
			Model:   model,
			SiteURL: siteURL,
			AppName: appName,
...
type OpenRouterEmbeddingProvider struct {
	inner   *openai.OpenAIEmbeddingProvider
