...
func TestProviderRegistry(t *testing.T) {
	...
	t.Run("GetBestLLMProvider", func(t *testing.T) {
		...
		config1 := DefaultProviderConfig(ProviderOpenAI)
		config1.APIKey = "test-key-1"
		err := registry.RegisterLLMProvider("openai", config1, 1)
		require.NoError(t, err)

		config2 := DefaultProviderConfig(ProviderOllama)
	...
	t.Run("GetBestEmbeddingProvider", func(t *testing.T) {
		...
		// Register multiple providers with different priorities
		config1 := DefaultEmbeddingProviderConfig(EmbeddingProviderOpenAI)
		config1.APIKey = "test-key-1"
		err := registry.RegisterEmbeddingProvider("openai", config1, 1)
		require.NoError(t, err)

		config2 := DefaultEmbeddingProviderConfig(EmbeddingProviderOllama)
	...
	t.Run("UpdateEmbeddingProviderConfig", func(t *testing.T) {
		...
		// Update configuration
		newConfig := DefaultEmbeddingProviderConfig(EmbeddingProviderOpenAI)
		newConfig.APIKey = "test-key"
	...
	t.Run("HealthCheck", func(t *testing.T) {
		...
		llmHealth := status.LLMProviders["openai"]
		assert.NotNil(t, llmHealth)
		assert.True(t, llmHealth.IsHealthy)

		embeddingHealth := status.EmbeddingProviders["openai-embedding"]
	...
	t.Run("SetLoadBalancing", func(t *testing.T) {
		...
		registry.SetLoadBalancing(LoadBalanceRoundRobin)
		registry.SetEmbeddingLoadBalancing(EmbeddingLoadBalanceRoundRobin)
...
func TestProviderRegistryIntegration(t *testing.T) {
	t.Run("CompleteWorkflow", func(t *testing.T) {
		...
		status, err := registry.HealthCheck(ctx)
		require.NoError(t, err)
		assert.True(t, status.LLMProviders["openai"].IsHealthy)
		assert.True(t, status.EmbeddingProviders["openai-embedding"].IsHealthy)

		// 6. Update configuration
		newLLMConfig := DefaultProviderConfig(ProviderOpenAI)
	...
	t.Run("MultiProviderFailover", func(t *testing.T) {
		...
		config3 := DefaultProviderConfig(ProviderDeepSeek)
