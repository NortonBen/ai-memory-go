// Package extractor provides health check and failover logic for LLM and embedding providers
package extractor
...
func (hc *HealthChecker) Start(ctx context.Context, manager ProviderManager) {
	hc.mu.Lock()
	if !hc.enabled {
		hc.mu.Unlock()
		return
...
func (hc *HealthChecker) StartEmbedding(ctx context.Context, manager EmbeddingProviderManager) {
	hc.mu.Lock()
	if !hc.enabled {
		hc.mu.Unlock()
		return
...
func (hc *HealthChecker) healthCheckWorker(ctx context.Context, manager ProviderManager) {
	defer hc.wg.Done()
...
func (hc *HealthChecker) embeddingHealthCheckWorker(ctx context.Context, manager EmbeddingProviderManager) {
	defer hc.wg.Done()
...
func (hc *HealthChecker) performHealthChecks(ctx context.Context, manager ProviderManager) {
...
func (hc *HealthChecker) performEmbeddingHealthChecks(ctx context.Context, manager EmbeddingProviderManager) {
	checkCtx, cancel := context.WithTimeout(ctx, hc.checkTimeout)
	defer cancel()
...
func (fm *FailoverManager) ExecuteWithFailover(
	...
	if !enabled {
		...
		if len(providers) == 0 {
			return NewExtractorError("no_providers", "no providers available", 503)
		}
	...
	return NewExtractorError("all_providers_failed", "all providers failed", 503)
}
...
func (fm *FailoverManager) ExecuteEmbeddingWithFailover(
	...
	if !enabled {
		// If failover is disabled, just try the first provider
		if len(providers) == 0 {
			return NewExtractorError("no_providers", "no embedding providers available", 503)
		}
		provider, err := manager.GetProvider(providers[0])
		if err != nil {
			return err
	...
	return NewExtractorError("all_providers_failed", "all embedding providers failed", 503)
}
...
func (fm *FailoverManager) isRetryableError(err error) bool {
	...
	extractorErr, ok := err.(*ExtractorError)
	if ok {
