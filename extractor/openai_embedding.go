// Package extractor - OpenAI embedding provider implementation
package extractor
...
// OpenAIEmbeddingProvider implements EmbeddingProvider for OpenAI embedding models
...
func NewOpenAIEmbeddingProvider(apiKey, model string) (*OpenAIEmbeddingProvider, error) {
	if apiKey == "" {
		return nil, NewExtractorError("validation", "OpenAI API key is required", 400)
	}
...
func NewOpenAIEmbeddingProviderFromConfig(config *EmbeddingProviderConfig) (*OpenAIEmbeddingProvider, error) {
...
func (oep *OpenAIEmbeddingProvider) GenerateEmbedding(ctx context.Context, text string) ([]float32, error) {
	...
	if len(resp.Data) == 0 {
		return nil, NewExtractorError("no_response", "OpenAI returned no embeddings", 500)
	}
...
func (oep *OpenAIEmbeddingProvider) GenerateBatchEmbeddings(ctx context.Context, texts []string) ([][]float32, error) {
	...
	if len(resp.Data) != len(texts) {
		return nil, NewExtractorError("invalid_response", fmt.Sprintf("expected %d embeddings, got %d", len(texts), len(resp.Data)), 500)
	}
...
func (oep *OpenAIEmbeddingProvider) GenerateEmbeddingWithOptions(ctx context.Context, text string, options *EmbeddingOptions) ([]float32, error) {
...
func (oep *OpenAIEmbeddingProvider) GenerateBatchEmbeddingsWithOptions(ctx context.Context, texts []string, options *EmbeddingOptions) ([][]float32, error) {
	// For OpenAI, options mainly affect dimensions
	if options != nil && options.Dimensions > 0 {
		// Temporarily set dimensions
		originalDims := oep.dimensions
...
func (oep *OpenAIEmbeddingProvider) GetProviderType() EmbeddingProviderType {
...
func (oep *OpenAIEmbeddingProvider) EstimateTokenCount(text string) (int, error) {
...
func (oep *OpenAIEmbeddingProvider) GetUsage(ctx context.Context) (*EmbeddingUsageStats, error) {
	oep.mu.RLock()
	defer oep.mu.RUnlock()
...
func (oep *OpenAIEmbeddingProvider) GetRateLimit(ctx context.Context) (*EmbeddingRateLimitStatus, error) {
...
func (oep *OpenAIEmbeddingProvider) Configure(config *EmbeddingProviderConfig) error {
	if err := ValidateEmbeddingProviderConfig(config); err != nil {
		return err
	}
...
func (oep *OpenAIEmbeddingProvider) GetConfiguration() *EmbeddingProviderConfig {
	oep.mu.RLock()
	defer oep.mu.RUnlock()
...
func (oep *OpenAIEmbeddingProvider) ValidateConfiguration(config *EmbeddingProviderConfig) error {
...
func (oep *OpenAIEmbeddingProvider) GenerateStreamingEmbedding(ctx context.Context, text string, callback EmbeddingStreamCallback) error {
	return NewExtractorError("unsupported", "OpenAI embedding provider does not support streaming", 501)
}
...
func (oep *OpenAIEmbeddingProvider) SetCustomDimensions(dimensions int) error {
	if !oep.SupportsCustomDimensions() {
		return NewExtractorError("unsupported", fmt.Sprintf("model %s does not support custom dimensions", oep.model), 400)
	}
	...
	if dimensions < 1 || dimensions > maxDims {
		return NewExtractorError("validation", fmt.Sprintf("dimensions must be between 1 and %d", maxDims), 400)
	}
...
func (oep *OpenAIEmbeddingProvider) GetCapabilities() *EmbeddingProviderCapabilities {
