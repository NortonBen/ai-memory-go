...
func TestFailoverManager(t *testing.T) {
	...
	t.Run("Identify retryable errors", func(t *testing.T) {
		...
		retryableErrors := []error{
			NewExtractorError("rate_limit", "rate limited", 429),
			NewExtractorError("timeout", "request timeout", 408),
			NewExtractorError("temporary_failure", "temporary failure", 503),
			NewExtractorError("health_check", "health check failed", 503),
		...
		nonRetryableErrors := []error{
			NewExtractorError("validation", "invalid input", 400),
			NewExtractorError("not_found", "not found", 404),
			NewExtractorError("unauthorized", "unauthorized", 401),
...
func TestFailoverIntegration(t *testing.T) {
	t.Run("Failover with multiple providers", func(t *testing.T) {
		...
		provider1 := newConfiguredMockLLMProvider(ProviderOpenAI, config1)

		config2 := DefaultProviderConfig(ProviderAnthropic)
		config2.APIKey = "test-key-2"
		provider2 := newConfiguredMockLLMProvider(ProviderAnthropic, config2)
	...
	t.Run("Failover on provider failure", func(t *testing.T) {
		...
		err := manager.(*DefaultProviderManager).ExecuteWithFailover(ctx, func(provider LLMProvider) error {
			callCount++
			if callCount == 1 {
				return NewExtractorError("temporary_failure", "provider unavailable", 503)
			}
			return nil
		})
