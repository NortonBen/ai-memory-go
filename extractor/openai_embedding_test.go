// Package extractor - OpenAI embedding provider tests
package extractor
...
func TestOpenAIEmbeddingProvider_EstimateTokenCount(t *testing.T) {
	provider, err := NewOpenAIEmbeddingProvider("test-api-key", string(openai.SmallEmbedding3))
	require.NoError(t, err)
...
func TestOpenAIEmbeddingProvider_ValidateConfiguration(t *testing.T) {
	...
	validConfig := DefaultEmbeddingProviderConfig(EmbeddingProviderOpenAI)
	validConfig.APIKey = "test-key"
	err = provider.ValidateConfiguration(validConfig)
	assert.NoError(t, err)

	invalidConfig := &EmbeddingProviderConfig{}
