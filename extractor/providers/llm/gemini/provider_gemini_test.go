...
func TestNewGeminiProvider(t *testing.T) {
	...
	for _, tt := range tests {
		t.Run(tt.name, func(t *testing.T) {
			...
			if provider.GetProviderType() != extractor.ProviderGemini {
				t.Errorf("NewGeminiProvider() provider type = %v, want %v", provider.GetProviderType(), extractor.ProviderGemini)
			}
		})
	}
}
...
func TestGeminiProvider_GetConfiguration(t *testing.T) {
	...
	if config.Type != extractor.ProviderGemini {
		t.Errorf("GetConfiguration() type = %v, want %v", config.Type, extractor.ProviderGemini)
	}
...
func TestGeminiProvider_Integration(t *testing.T) {
	...
	t.Run("GenerateWithContext", func(t *testing.T) {
		messages := []extractor.Message{
			...
			{Role: extractor.RoleUser, Content: "What is 2+2?"},
		}
...
func TestNewGeminiEmbeddingProvider(t *testing.T) {
	...
	for _, tt := range tests {
		t.Run(tt.name, func(t *testing.T) {
			...
			if provider.GetProviderType() != extractor.EmbeddingProviderGemini {
				t.Errorf("NewGeminiEmbeddingProvider() provider type = %v, want %v", provider.GetProviderType(), extractor.EmbeddingProviderGemini)
			}
...
func TestGeminiEmbeddingProvider_GetTokenCount(t *testing.T) {
	provider, err := NewGeminiEmbeddingProvider("test-api-key", TextEmbedding004)
	if err != nil {
		t.Fatalf("NewGeminiEmbeddingProvider() error: %v", err)
	}
...
func TestGeminiEmbeddingProvider_Integration(t *testing.T) {
	...
	t.Run("GenerateEmbeddingWithOptions", func(t *testing.T) {
		...
		options := &extractor.EmbeddingOptions{
			Dimensions: 512,
		}
