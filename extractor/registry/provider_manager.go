...
// DefaultProviderManager implements the ext.ProviderManager interface
type DefaultProviderManager struct {
	mu                  sync.RWMutex
	providers           map[string]*ext.ProviderEntry
	failoverEnabled     bool
	loadBalancing       ext.LoadBalancingStrategy
...
// ProviderEntry and other core structs moved to extractor/extractor.go

// NewProviderManager creates a new provider manager
func NewProviderManager() ext.ProviderManager {
	return &DefaultProviderManager{
		providers:           make(map[string]*ext.ProviderEntry),
		failoverEnabled:     true,
		loadBalancing:       ext.LoadBalancePriority,
...
func (m *DefaultProviderManager) AddProvider(name string, provider ext.LLMProvider, priority int) error {
	if provider == nil {
		return ext.NewExtractorError("validation", "provider is nil", 400)
	}

	m.mu.Lock()
	defer m.mu.Unlock()

	m.providers[name] = &ext.ProviderEntry{
		Provider: provider,
		Priority: priority,
...
func (m *DefaultProviderManager) RemoveProvider(name string) error {
	...
	if !exists {
		return ext.NewExtractorError("not_found", fmt.Sprintf("provider %s not found", name), 404)
	}
...
func (m *DefaultProviderManager) GetProvider(name string) (ext.LLMProvider, error) {
	m.mu.RLock()
	defer m.mu.RUnlock()

	entry, exists := m.providers[name]
	if !exists {
		return nil, ext.NewExtractorError("not_found", fmt.Sprintf("provider %s not found", name), 404)
	}
...
func (m *DefaultProviderManager) GetBestProvider(ctx context.Context) (ext.LLMProvider, error) {
	m.mu.Lock()
	defer m.mu.Unlock()

	if len(m.providers) == 0 {
		return nil, ext.NewExtractorError("no_providers", "no providers available", 503)
	}

	// Get healthy providers
	var healthyProviders []*ext.ProviderEntry
	...
	if len(healthyProviders) == 0 {
		return nil, ext.NewExtractorError("no_healthy_providers", "no healthy providers available", 503)
	}

	// Select provider based on load balancing strategy
	var selectedEntry *ext.ProviderEntry
	switch m.loadBalancing {
	case ext.LoadBalancePriority:
		selectedEntry = m.selectByPriority(healthyProviders)
	case ext.LoadBalanceRoundRobin:
		selectedEntry = m.selectByRoundRobin(healthyProviders)
	case ext.LoadBalanceRandom:
		selectedEntry = m.selectByRandom(healthyProviders)
	case ext.LoadBalanceLeastUsed:
	...
	if selectedEntry == nil {
		return nil, ext.NewExtractorError("selection_failed", "failed to select provider", 500)
	}
...
func (m *DefaultProviderManager) ListProviders() map[string]ext.LLMProvider {
	m.mu.RLock()
	defer m.mu.RUnlock()

	result := make(map[string]ext.LLMProvider)
...
func (m *DefaultProviderManager) SetLoadBalancing(strategy ext.LoadBalancingStrategy) {
	m.mu.Lock()
	defer m.mu.Unlock()
...
func (m *DefaultProviderManager) selectByPriority(providers []*ext.ProviderEntry) *ext.ProviderEntry {
	if len(providers) == 0 {
		return nil
	}
...
func (m *DefaultProviderManager) selectByRoundRobin(providers []*ext.ProviderEntry) *ext.ProviderEntry {
	if len(providers) == 0 {
		return nil
	}
...
func (m *DefaultProviderManager) selectByRandom(providers []*ext.ProviderEntry) *ext.ProviderEntry {
	if len(providers) == 0 {
		return nil
	}
...
func (m *DefaultProviderManager) selectByLeastUsed(providers []*ext.ProviderEntry) *ext.ProviderEntry {
	if len(providers) == 0 {
		return nil
	}
...
// DefaultEmbeddingProviderManager implements the ext.EmbeddingProviderManager interface
type DefaultEmbeddingProviderManager struct {
	mu                  sync.RWMutex
	providers           map[string]*ext.EmbeddingProviderEntry
	failoverEnabled     bool
	loadBalancing       ext.EmbeddingLoadBalancingStrategy
	roundRobinCounter   int
	healthChecker       *HealthChecker
	failoverManager     *FailoverManager
	availabilityTracker *ProviderAvailabilityTracker
...
// EmbeddingProviderEntry and other core structs moved to extractor/extractor.go

// NewEmbeddingProviderManager creates a new embedding provider manager
func NewEmbeddingProviderManager() ext.EmbeddingProviderManager {
	return &DefaultEmbeddingProviderManager{
		providers:           make(map[string]*ext.EmbeddingProviderEntry),
		failoverEnabled:     true,
		loadBalancing:       ext.EmbeddingLoadBalancePriority,
		healthChecker:       NewHealthChecker(5*time.Minute, 30*time.Second, 3),
		failoverManager:     NewFailoverManager(),
		availabilityTracker: NewProviderAvailabilityTracker(1*time.Hour, 0.8),
	}
...
func (m *DefaultEmbeddingProviderManager) AddProvider(name string, provider ext.EmbeddingProvider, priority int) error {
	if provider == nil {
		return ext.NewExtractorError("validation", "embedding provider is nil", 400)
	}

	m.mu.Lock()
	defer m.mu.Unlock()

	m.providers[name] = &ext.EmbeddingProviderEntry{
		Provider: provider,
		Priority: priority,
...
func (m *DefaultEmbeddingProviderManager) GetProvider(name string) (ext.EmbeddingProvider, error) {
	m.mu.RLock()
	defer m.mu.RUnlock()
...
func (m *DefaultEmbeddingProviderManager) GetBestProvider(ctx context.Context) (ext.EmbeddingProvider, error) {
	m.mu.Lock()
	defer m.mu.Unlock()

	if len(m.providers) == 0 {
		return nil, ext.NewExtractorError("no_providers", "no embedding providers available", 503)
	}

	// Get healthy providers
	var healthyProviders []*ext.EmbeddingProviderEntry
	for _, entry := range m.providers {
		if entry.Health.IsHealthy {
			healthyProviders = append(healthyProviders, entry)
		}
	...
	if len(healthyProviders) == 0 {
		return nil, ext.NewExtractorError("no_healthy_providers", "no healthy embedding providers available", 503)
	}

	// Select provider based on load balancing strategy
	var selectedEntry *ext.EmbeddingProviderEntry
	switch m.loadBalancing {
	case ext.EmbeddingLoadBalancePriority:
		selectedEntry = m.selectEmbeddingByPriority(healthyProviders)
	case ext.EmbeddingLoadBalanceRoundRobin:
		selectedEntry = m.selectEmbeddingByRoundRobin(healthyProviders)
	case ext.EmbeddingLoadBalanceRandom:
		selectedEntry = m.selectEmbeddingByRandom(healthyProviders)
	case ext.EmbeddingLoadBalanceLeastUsed:
		selectedEntry = m.selectEmbeddingByLeastUsed(healthyProviders)
	case ext.EmbeddingLoadBalanceCostBased:
		selectedEntry = m.selectEmbeddingByCost(healthyProviders)
	case ext.EmbeddingLoadBalanceLatency:
	...
	if selectedEntry == nil {
		return nil, ext.NewExtractorError("selection_failed", "failed to select embedding provider", 500)
	}
...
func (m *DefaultEmbeddingProviderManager) ListProviders() map[string]ext.EmbeddingProvider {
	m.mu.RLock()
	defer m.mu.RUnlock()

	result := make(map[string]ext.EmbeddingProvider)
	for name, entry := range m.providers {
		result[name] = entry.Provider
	}
	return result
...
func (m *DefaultEmbeddingProviderManager) SetLoadBalancing(strategy ext.EmbeddingLoadBalancingStrategy) {
	m.mu.Lock()
	defer m.mu.Unlock()
	m.loadBalancing = strategy
}
...
func (m *DefaultEmbeddingProviderManager) GenerateEmbeddingWithFailover(ctx context.Context, text string) ([]float32, error) {
	...
	m.mu.RLock()
	var providers []*ext.EmbeddingProviderEntry
	for _, entry := range m.providers {
	...
	return nil, ext.NewExtractorError("all_providers_failed", "all embedding providers failed", 503)
}
...
func (m *DefaultEmbeddingProviderManager) selectEmbeddingByPriority(providers []*ext.EmbeddingProviderEntry) *ext.EmbeddingProviderEntry {
	if len(providers) == 0 {
		return nil
	}
...
func (m *DefaultEmbeddingProviderManager) selectEmbeddingByRoundRobin(providers []*ext.EmbeddingProviderEntry) *ext.EmbeddingProviderEntry {
	if len(providers) == 0 {
		return nil
	}
...
func (m *DefaultEmbeddingProviderManager) selectEmbeddingByRandom(providers []*ext.EmbeddingProviderEntry) *ext.EmbeddingProviderEntry {
	if len(providers) == 0 {
		return nil
	}
...
func (m *DefaultEmbeddingProviderManager) selectEmbeddingByLeastUsed(providers []*ext.EmbeddingProviderEntry) *ext.EmbeddingProviderEntry {
	if len(providers) == 0 {
		return nil
	}
...
func (m *DefaultEmbeddingProviderManager) selectEmbeddingByCost(providers []*ext.EmbeddingProviderEntry) *ext.EmbeddingProviderEntry {
	if len(providers) == 0 {
		return nil
	}
...
func (m *DefaultEmbeddingProviderManager) selectEmbeddingByLatency(providers []*ext.EmbeddingProviderEntry) *ext.EmbeddingProviderEntry {
	if len(providers) == 0 {
		return nil
	}
...
func (m *DefaultEmbeddingProviderManager) ExecuteWithFailover(
	...
	// Get provider names sorted by priority
	var providerNames []string
	var entries []*ext.EmbeddingProviderEntry
	for name, entry := range m.providers {
		providerNames = append(providerNames, name)
		entries = append(entries, entry)
	}
