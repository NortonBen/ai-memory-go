...
func init() {
	graph.RegisterStore(graph.StoreTypeRedis, func(config *graph.GraphConfig) (graph.GraphStore, error) {
		return NewRedisGraphStore(config)
	})
}
...
func NewRedisGraphStore(config *graph.GraphConfig) (*RedisGraphStore, error) {
