// Package core - Enhanced file type detection and routing
package core
...
func (ftd *FileTypeDetector) DetectFileInfo(filePath string) (*FileInfo, error) {
	...
	info.Format = formats.DetectFileFormat(filePath)
...
type RouterConfig struct {
	...
	CustomParsers        map[string]schema.Parser `json:"-"`
...
func DefaultRouterConfig() *RouterConfig {
	return &RouterConfig{
		...
		CustomParsers:     make(map[string]schema.Parser),
		FallbackToText:    true,
...
type FileRouter struct {
	...
	parsers  map[string]schema.Parser
}
...
func NewFileRouter(config *RouterConfig, chunkingConfig *schema.ChunkingConfig) *FileRouter {
	...
	router := &FileRouter{
		...
		parsers:  make(map[string]schema.Parser),
	}

	// Register default formats
	router.RegisterParser("txt", formats.NewFormatParser(chunkingConfig))
	router.RegisterParser("csv", formats.NewFormatParser(chunkingConfig))
	router.RegisterParser("json", formats.NewFormatParser(chunkingConfig))
	router.RegisterParser("markdown", formats.NewTextParser(chunkingConfig))
	router.RegisterParser("pdf", formats.NewPDFParser(chunkingConfig))

	// Register custom parsers
	for format, p := range config.CustomParsers {
		router.RegisterParser(format, p)
	}
...
func (fr *FileRouter) RegisterParser(format string, p schema.Parser) {
	fr.parsers[format] = p
}
...
func (fr *FileRouter) RouteFile(ctx context.Context, filePath string) ([]*schema.Chunk, error) {
	...
	p, exists := fr.parsers[fileInfo.Format]
	if !exists {
		if fr.config.FallbackToText {
			p = fr.parsers["txt"]
		} else {
	...
	// Parse the file
	chunks, err := fr.parseWithParser(ctx, p, filePath, fileInfo)
	if err != nil {
...
func (fr *FileRouter) parseWithParser(ctx context.Context, p schema.Parser, filePath string, fileInfo *FileInfo) ([]*schema.Chunk, error) {
	switch fileInfo.Format {
	case "pdf":
		if pdfParser, ok := p.(*formats.PDFParser); ok {
			return pdfParser.ParsePDF(ctx, filePath)
		}
	case "csv":
		if formatParser, ok := p.(*formats.FormatParser); ok {
	...
	case "markdown":
		if textParser, ok := p.(*formats.TextParser); ok {
...
func (fr *FileRouter) GetSupportedFormats() []string {
	supported := make([]string, 0, len(fr.parsers))
