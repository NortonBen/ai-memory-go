// Package core - Unified multi-format parser implementation
package core
...
type UnifiedParser struct {
	...
	textParser      *formats.TextParser
	pdfParser       *formats.PDFParser
	formatParser    *formats.FormatParser
	streamingParser *stream.StreamingParser
...
func NewUnifiedParser(config *schema.ChunkingConfig) *UnifiedParser {
	...
	p := &UnifiedParser{
		config:       config,
		textParser:   formats.NewTextParser(config),
		pdfParser:    formats.NewPDFParser(config),
		formatParser: formats.NewFormatParser(config),
	}

	p.streamingParser = stream.NewStreamingParser(stream.DefaultStreamingConfig(), config)

	// Initialize worker pool with default config
	p.workerPool = concurrency.NewWorkerPool(p, concurrency.DefaultWorkerPoolConfig())

	return p
}

// NewUnifiedParserWithConfigs creates a new unified parser with explicit configurations
func NewUnifiedParserWithConfigs(chunkConfig *schema.ChunkingConfig, workerPoolConfig *schema.WorkerPoolConfig, streamingConfig *schema.StreamingConfig) *UnifiedParser {
	if chunkConfig == nil {
		chunkConfig = schema.DefaultChunkingConfig()
	}
	if workerPoolConfig == nil {
		workerPoolConfig = concurrency.DefaultWorkerPoolConfig()
	}
	if streamingConfig == nil {
		streamingConfig = stream.DefaultStreamingConfig()
	}

	p := &UnifiedParser{
		config:       chunkConfig,
		textParser:   formats.NewTextParser(chunkConfig),
		pdfParser:    formats.NewPDFParser(chunkConfig),
		formatParser: formats.NewFormatParser(chunkConfig),
	}

	p.streamingParser = stream.NewStreamingParser(streamingConfig, chunkConfig)
	p.workerPool = concurrency.NewWorkerPool(p, workerPoolConfig)

	return p
}

// NewUnifiedParserWithWorkerPool creates a new unified parser with a specific worker pool configuration
func NewUnifiedParserWithWorkerPool(chunkConfig *schema.ChunkingConfig, workerPoolConfig *schema.WorkerPoolConfig) *UnifiedParser {
	return NewUnifiedParserWithConfigs(chunkConfig, workerPoolConfig, nil)
}
...
func (up *UnifiedParser) ParseFile(ctx context.Context, filePath string) ([]*schema.Chunk, error) {
	...
	// Detect file format
	format := formats.DetectFileFormat(filePath)
...
func (up *UnifiedParser) ProcessFilesParallel(ctx context.Context, filePaths []string, metadata map[string]interface{}) (map[string][]*schema.Chunk, error) {
	...
	// Submit all tasks
	tasks := make([]*concurrency.ProcessingTask, 0, len(filePaths))
