...
func TestUnifiedParserStreamingIntegration(t *testing.T) {
	...
	assert.NotNil(t, parser.streamingParser)

	// Test streaming config access
...
func TestUnifiedParserStreamingMemoryUsage(t *testing.T) {
	parser := NewUnifiedParser(nil)
	defer parser.Close()

	memUsage := parser.GetStreamingMemoryUsage()
	assert.Greater(t, memUsage, int64(0))

	// Should be reasonable for default config
	expectedUsage := int64(64*1024 + 4*1024 + 1024) // Buffer + MaxChunk + Overlap
...
func TestUnifiedParserStreamingWithCustomChunkingConfig(t *testing.T) {
	...
	parser := NewUnifiedParser(chunkConfig)
	defer parser.Close()
...
func TestUnifiedParserStreamingWithWorkerPool(t *testing.T) {
	// Create parser with custom worker pool config
	workerConfig := &WorkerPoolConfig{
		NumWorkers: 2,
		QueueSize:  10,
	}

	parser := NewUnifiedParserWithWorkerPool(nil, workerConfig)
	defer parser.Close()

	// Verify streaming parser is still available
	assert.NotNil(t, parser.streamingParser)

	// Test streaming functionality
	content := generateStreamTestContent(200)
	reader := strings.NewReader(content)

	result, err := parser.ParseReaderStream(context.Background(), reader, "test_worker_pool")
	require.NoError(t, err)
	assert.Greater(t, len(result.Chunks), 0)
}
...
func TestUnifiedParserStreamingPerformanceComparison(t *testing.T) {
	...
	largeContent := generateStreamTestContent(2000) // Large content
	...
	// Both should produce similar number of chunks
	assert.InDelta(t, len(regularChunks), len(streamResult.Chunks), float64(len(regularChunks))*0.2)

	// Streaming should have reasonable performance metrics
	assert.Greater(t, streamResult.ProcessingTime.Nanoseconds(), int64(0))
	assert.Greater(t, streamResult.TotalBytes, int64(0))
}
...
// Benchmark streaming vs regular parsing
func BenchmarkUnifiedParserStreamingVsRegular(b *testing.B) {
	content := generateStreamTestContent(1000) // Large content
...
func BenchmarkUnifiedParserAutoSelection(b *testing.B) {
	parser := NewUnifiedParser(nil)
	defer parser.Close()
	...
	smallFile := filepath.Join(tmpDir, "small.txt")
	smallContent := generateStreamTestContent(10)
	os.WriteFile(smallFile, []byte(smallContent), 0644)

	largeFile := filepath.Join(tmpDir, "large.txt")
	largeContent := strings.Repeat(generateStreamTestContent(100), 100) // Very large
	os.WriteFile(largeFile, []byte(largeContent), 0644)

	b.Run("AutoSmall", func(b *testing.B) {
		b.ResetTimer()
		for i := 0; i < b.N; i++ {
			_, err := parser.ParseFileAuto(context.Background(), smallFile)
			if err != nil {
				b.Fatal(err)
			}
		}
	...
	b.Run("AutoLarge", func(b *testing.B) {
		b.ResetTimer()
		for i := 0; i < b.N; i++ {
			_, err := parser.ParseFileAuto(context.Background(), largeFile)
			if err != nil {
				b.Fatal(err)
			}
		}
