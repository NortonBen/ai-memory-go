...
func BenchmarkAllParserTypes(b *testing.B) {
	...
	b.Run("CachedParser", func(b *testing.B) {
		parser := core.NewCachedUnifiedParser(schema.DefaultChunkingConfig(), cache.DefaultCacheConfig())
		defer parser.Close()
	...
	b.Run("StreamingParser", func(b *testing.B) {
		parser := stream.NewStreamingParser(stream.DefaultStreamingConfig(), schema.DefaultChunkingConfig())
...
func BenchmarkConfigurationComparison(b *testing.B) {
	...
	for _, cfg := range configs {
		b.Run(cfg.name, func(b *testing.B) {
			parser := core.NewUnifiedParser(cfg.config)
			defer parser.Close()
